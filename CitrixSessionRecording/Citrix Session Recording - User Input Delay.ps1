<#
    ...
    .PARAMETER UserInputDelay

    .EXAMPLE
        . .\AnnotateCitrixSession.ps1 -SessionId 4 -UserInputDelay 300
        Record an event for session Id 4 that the User Input Delay was 300ms.
...
Param (
    ...
    [Parameter(Mandatory=$true,HelpMessage='User Input Delay Value')][ValidateNotNullOrEmpty()]                                                    [int]$UserInputDelay
)
...
#Annotate events
$newLogger.LogData($SessionId, "ControlUp","AA","Trigger","High Stress User Input Delay: $($UserInputDelay)ms",[byte]0,$true)
Write-Output -Message "High Stress User Input Delay $($UserInputDelay)ms for $SessionID"  #Write-Output for ControlUp Auditing to capture