<#
    ...
    .PARAMETER NetworkLatency

    .EXAMPLE
        . .\AnnotateCitrixSession.ps1 -SessionId 4 -NetworkLatency 400
...
Param (
    ...
    [Parameter(Mandatory=$true,HelpMessage='SessionState')][ValidateNotNullOrEmpty()]                                                    [string]$NetworkLatency
)
...
#Annotate events
$newLogger.LogData($SessionId, "ControlUp","AA","Trigger","High Stress Network Latency: $($NetworkLatency)ms",[byte]0,$true)
Write-Output -Message "High Stress Network Latency: $($NetworkLatency)ms"  #Write-Output for ControlUp Auditing to capture